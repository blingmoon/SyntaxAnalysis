...
namespace base_grammar{
    ...
    public://属性
        ...
        bool is_finally;//判断是不终结符，是则为true;
    public://方法

        //给这个Elem添加产生式
        //bool add_expression(expr expression);
        //构造函数
...
namespace public_tool{
    ...
    //?作为转义字符,区别|和?|
    vector<string>& parse_productions(string EX, vector<string>& productions);

    //单独解析,Ex为BAB,结果为B,A,B
    //@表示空串,$...$表示...是一个整体，否则一个字符串一个字符串的解析
    vector<string>& parse_elem(string production, vector<string>& elems);

    bool is_in_vector(const std::vector<int>& a, int b);
    void connect_vector(std::vector<int>& a, const std::vector<int>& b);
